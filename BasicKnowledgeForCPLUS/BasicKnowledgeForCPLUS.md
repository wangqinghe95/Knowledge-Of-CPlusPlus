...
### 合成的默认构造函数中有哪些需要注意的点？
1. 不会初始化内置类型和复合类型的数据成员，复合类型数据成员包括数组，结构体，共用体，引用和指针
2. 只有在需要的时候才会生成，需要指的是 **什么情况会生成默认构造函数**
3. 只初始化类对象数据成员和基类子对象。初始化复合类型的数据成员和类对象数据成员是两回事。
4. 如果类中已经定义了构造函数，编译器就会不再生成默认构造函数。
...
### 虚函数的代价？
+ 空间开销：每个包含虚函数的类都会有一个虚函数表和虚函数指针，都会额外占用实例的空间；
+ 时间开销：虚函数的调用都需要通过虚函数指针，再到虚函数表，再计算偏移量找到虚函数地址并调用，相当于增加了一次内存寻址。
+ 优化限制：由于虚函数的地址在运行时才会确定，编译器很难对虚函数进行优化。
+ 分支预测：因为虚函数调用需要间接跳转，这会导致虚函数调用会比普通函数调用多了分支预测的过程，如果分支预测失败，就会导致流水线被冲刷，重新进行取值，译码等操作，这对程序性能有很大影响。
...
### 虚拟继承和虚继承
1. 虚拟继承和虚继承是同一概念的不同表述，都是 C++ 中用于实现虚基类的机制。
2. 虚基类是为了解决多重继承时的命名冲突和冗余数据问题，使得派生类中只保留一份间接基类的成员，如果不使用虚继承，每个路径都会在派生类中包含一份基类实例，造成数据冗余和访问冲突。
3. 在虚继承中，派生类通过关键字 virtual 来声明对虚基类的继承，而不是直接继承。这样在派生类中就保留了一份成员变量，直接访问就不会有歧义。
...
### 介绍一下重载运算符？
   ...
   2. 重载运算符是通过使用关键字 operator 修饰一个运算符符号实现。
   3. 重载的运算符是成员函数时，this 绑定在左侧运算符对象，所以成员运算符函数参数比非成员函数运算符成员函数少一个；
