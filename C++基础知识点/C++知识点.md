...
+ 引用传递和指针传递是不同的
    - 虽然它们都是在被调函数栈空间上的一个局部变量，但是任何对于引用参数参数的处理都会通过一个间接寻址的方式操作到主调函数中的相关变量；
    - 而对于指针传递的参数，如果改变被调函数中的指针地址，它将应用不到主调函数的相关变量。如果想要通过指针参数传递来改变主调函数中的相关变量（地址—），那就的使用指向指针的指针或者指针引用了；
...
## 什么是虚函数？虚函数和纯虚函数的区别？
+ 虚函数是 C++ 用于实现多态的机制，核心理念就是通过基类指针访问派生类定义的函数；
+ 虚函数和纯虚函数的区别：
...
+ vptr 虚表指针是在类在进行实例化时，构造函数的执行时进行初始化，存在对象内存布局的最前面；
...
## 介绍面对对象的三大特性，并举例说明？
+ 把对象的服务和属性结合成一个独立的服务单元，并尽可能的隐藏内部的实现细节；
...
+ 神拷贝不仅是拷贝值，还会开辟一块新的空间区存放新的值，即使原先的对象被释放掉，释放内存了也不会影响深拷贝的值了。
...
+ 乡下类型转换：将基类指针或引用转换为派生类指针或引用被称为乡下类型转换，向下类型转换不会自动进行，因为一个基类对应几个派生类，所以向下转换时不知道对应哪个派生类，所以在向下类型转换时必须加上动态类型时别技术。
...
5. 调用过程：编译器会一一操作初始化列表，以适当的顺序在构造函数之内安插初始化操作，并且在任何显示用户代码前，list中项目顺序是由类中的成员声明顺序决定的，而不是初始化列表中的排列顺序决定的；
