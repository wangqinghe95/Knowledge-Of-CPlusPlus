...
2. C++ 三大特性
   1. 封装、继承、多态。
3. 重载、重写、隐藏的区别
   1. 重载是子类对父类中被 virtual 修饰的函数的重新是现在，要求函数声明保持一致；
   2. 重写是类中两个函数名相同，但是函数参数，参数个数，参数类型有差别即可；
   3. 隐藏是指子类对父类同名但是没有被 virtual 修饰的函数的隐藏。
4. 什么是 RAII 技术？
   1. Resource Acquisition Is Initialization， 一种编程思想，意思是资源获取即初始化，不需要时正确释放
   2. 通过对象构造函数和析构函数实现。
...
2. 解释虚函数和纯虚函数
   1. 虚函数被 virtual 关键字修饰的类成员函数，是用来实现 C++ 多态的方法
   2. 纯虚函数是在虚函数后使用 =0 修饰的成员函数，表示该函数在基类只是没有实现，只是声明，实现由派生类做。
3. 抽象基类为什么不能创建对象？
   1. 面向对象语言中，为了更好的实现多态，需要有一个抽象的，不能被实例化的类，单纯地用来做基类。那么就引入了纯虚函数的定义，当一个类中的函数被设置为纯虚函数后，该类就成为抽象机会，不允许实例化。所以这个问题的因果应该是，不能被创建对象的类被称为抽象基类。
4. 什么是虚函数表（virtual function table）？它在 C++ 中有什么作用？
   1. C++ 编译器将一个类中的所有虚函数地址放入到一个常指针数组中，该数组就是虚函数表
   2. 用于实现多态。当一个类继承的父类有虚函数时，它也会将父类的虚函数也一并继承，如果重载了父类的虚函数，那么虚函数表中的对应虚函数地址也一并会替换掉。当基类指针指向派生类对象时，调用的虚函数根据子类是否重写来决定最终调用哪个函数，这种在运行时才决定调用函数的行为被称为多态。
5. 什么是 vtable 和 vptr？它们在 C++中有什么作用？
   1. 虚函数表是一个指针数组，用来存储一个类中所有虚函数地址，虚函数指针是指向该虚函数表的指针
   2. 两者都是为了实现 C++ 多态。
6. 静态函数能定义为虚函数吗？常函数呢？
   1. 不能，因为静态函数不是类对象拥有，它所属于类。常函数可以，因为它可以由 this 指针调用
7. 哪些函数不能定义为虚函数？
   1. 构造函数，虚函数指针在构造函数中初始化的
   2. 内联函数，内联函数在编译阶段在调用的地方展开，而虚函数在运行时进行函数调用确定，而且矛盾。
   3. 静态函数，不能被 this 指针调用
   4. 友元函数，友元函数不属于类的成员函数，无法被继承
8. 基类的虚函数表存储在内存的什么位置（哪个区）？虚表指针 vptr 的初始化位置？
   1. 虚函数表存放在只读数据段，虚表指针初始化在类对象存储地址的前 8 个字节。
9. 虚函数的代价？
   1. 增加空间开销，时间开销，分支预测失败开销和优化被限制代价。
10. 虚拟继承和虚继承？
   1. 两者都是 C++ 用来实现虚基类的机制，解决多重继承时命令冲突和冗余数据问题，使得派生类中只保留一份间接基类的成员。
11. 单继承的内存模型是什么样的？
   1. 单继承模式下，派生类复制基类的虚函数表，如果派生类中额外的虚函数，则继续在虚函数表后按照声明的顺序添加。最后再将派生类重新的虚函数地址覆盖基类的虚函数表中对应的虚函数地址。
12. 多继承的内存模型？
   1. 派生类按照基类的声明顺序，依次的将基类的虚函数表内容复制到自己的虚函数表中，重写的虚函数地址会覆盖对应的虚函数表中虚函数地址，最后再在虚函数表后增加自己的虚函数地址
13. 虚继承的内存模型？(棱形继承)
   1. 派生类会产生两个虚指针，一个是指向虚基表，一个指向自己的虚表
   2.  多重继承时，派生类中虚继承和虚基指针有且仅有一份
14. 多继承的优缺点
   1. 优点是可以从多个父类继承成员函数和成员变量，可以让子类拥有更高的丰富性。缺点是继承的父类过多可能会产生二义性，应用场景少，通常会使用组合替代多继承。
15. 解释编译时多态/静态多态和运行时多态/动态多态，它们有什么区别？
   1. 静态多态指的是在编译时期就能确定函数的调用地址，通常通过函数重载和模板实现。效率高灵活性差，只能处理已知的数据类型
   2. 动态多态指的是在运行时根据基类的引用指向的对象类型来决定调用哪一个虚函数。通常通过虚函数，继承等机制实现，灵活性高效率低。
16. 静态类型和动态类型、静态绑定和动态绑定的介绍？
   1. 对象在声明时定义的类型，在编译时期确定叫静态类型；一个指针或者引用所指向的对象类型在运行时期才决定的叫动态类型；
   2. 在编译时期绑定了静态类型叫静态绑定；在运行期绑定了动态类型叫动态绑定；
...
4. 什么是动态绑定，它的使用场景是什么？
   1. 动态绑定指的是在程序运行时根据对象的实际类型来选择调用的函数或方法。
   2. 在 C++ 中，虚函数，if，switch 都是动态绑定的实际应用
5. 引用是否能实现动态绑定，为什么可以实现？
   1. 可以，基类引用绑定了派生类对象，就能在引用调用虚函数时根据对象的实际类型调用函数或方法，从而实现动态绑定。
6. 继承机制中对象直接如何转换？
   1. 派生类指针转基类指针一般都是安全的，该转换自动进行；基类指针或引用转派生类有风险，用 dynamic_cast 转换
...
8. 为什么友元函数必须在内部声明？
   1. 因为编译器需要在编译器时期就需要知道哪些函数可以访问类的私有成员
9. 介绍一下重载运算符？
   1. 重载运算符是为了使得类或者枚举类型的对象可以想内置类型一样使用运算符的特性。通过重载运算符可以改变已有运算符的行为，或者赋予用户定义的类型使用运算符的能力。
   2. 重载的成员运算符中，this 指针会绑定到运算符左边。
10. 类静态成员和类普通成员的区别是什么？
   1. 静态成员从类加载开始存在于内存中，一直到类被卸载，且是所有类共享该静态成员变量，定义在静态全局区，类外初始化，作为默认实参；普通类成员变量只有当类实例化才会存在内存中；每个对象单独拥有一份普通成员变量，定义在堆或者栈区，在类中初始化；
11. 如何设计一个类来计算类实例化的个数
   1. 使用静态成员变量统计每次类新构造和析构时的次数变化；
12. 如何计算一个类的子类个数
   1. 使用反射机制计算子类的个数
13. 如何阻止一个类被实例化的方法？
   1. 把类定义为抽象类
...
17. 构造函数、析构函数、虚函数是否能声明为内联函数?
	1. 可以申明为内联，但是具体是否会内联还是要看 C++ 编译器自己决定。
18. 什么情况会自动生成默认构造函数？
	1. 含有一个拥有默认构造函数的类对象数据成员时，继承的基类带有默认构造函数时，带有虚函数的类，带有虚基类的类
19. 合成的默认构造函数中有哪些需要注意的点？
   1. 不会初始化内置类型和复合类型的数据成员，只有需要默认构造函数时才会被生成，只初始化类对象数据成员和基类子对象，如果类中已经有了构造函数，那么编译器就不会再生成默认构造函数
20. 什么时候需要合成拷贝构造函数？
   1. 一个类没有拷贝构造函数，但是有一个类类型的成员变量，该类有拷贝构造函数；该类继承有拷贝函数的基类；当该类对象会被当作函数参数时；当函数返回一个类对象时；对一个对象做显示地初始化操作时，编译器会给该类合成一个拷贝构造函数
21. 如何禁止程序自动生成拷贝构造函数？
   1. 将拷贝构造函数显示地声明为 private 权限
   2. 用 = delete 修饰
22. 什么是 trivial destructor
   1. 一个不执行任何操作的析构函数。适用在只有基本数据类型的类中。
